...
fn setup_test_output_dir() -> std::io::Result<()> {
    ...
    fs::create_dir_all(format!("{manifest_dir}/test_output"))
}
...
fn test_basic_diagram_generation() {
    ...
    assert!(
        ...
        "Output should contain MainActivity class"
    );
    assert!(
        content.contains("class Notification"),
        "Output should contain Notification class"
    );
    assert!(
        content.contains("class NotificationAdapter"),
        "Output should contain NotificationAdapter class"
    );
    assert!(
        content.contains("class NotificationRepository"),
        "Output should contain NotificationRepository class"
    );
    assert!(
        content.contains("class QRGenerator"),
        "Output should contain QRGenerator class"
    );
}
...
fn test_skip_annotation() {
    ...
    assert!(
        content.contains("class TestClass"),
    ...
    assert!(
        !content.contains("class SkippedClass"),
    ...
    assert!(
        content.contains("visibleField"),
        "Output should contain visibleField"
    );
    assert!(
        content.contains("visibleMethod"),
        "Output should contain visibleMethod"
    );

    // Should NOT contain hidden fields/methods marked with @Skip
    assert!(
        !content.contains("hiddenField"),
        "Output should not contain hiddenField (marked with @Skip)"
    );
    assert!(
        !content.contains("hiddenMethod"),
        "Output should not contain hiddenMethod (marked with @Skip)"
    );
}
...
fn test_class_retention_annotation() {
    ...
    assert!(
        content.contains("class TestClassRetention"),
        "Output should contain TestClassRetention"
    );
    ...
    assert!(
        !content.contains("hiddenFieldWithClassRetention"),
        "Output should not contain hiddenFieldWithClassRetention (marked with @SkipClass)"
    );
    assert!(
        !content.contains("hiddenMethodWithClassRetention"),
